...
export const FONERO_VERSION = "FONERO_VERSION";
...
export const DELETE_FNOD_ATTEMPT = "DELETE_FNOD_ATTEMPT";
export const DELETE_FNOD_FAILED = "DELETE_FNOD_FAILED";
export const DELETE_FNOD_SUCCESS = "DELETE_FNOD_SUCCESS";
  ...
  axios.get(releaseApiURL, { timeout: 5000 })
    .then(function (response) {
      ...
      if (semverCompatible(currentVersion, detectedVersion)) {
        wallet.log("info", "Fonero version up to date.");
      } else {
        dispatch({ type: FONERO_VERSION, msg:  response.data[0].tag_name });
      }
    })
    .catch(function (error) {
      console.log("Unable to check latest fonero release version.", error);
    });
};
...
export const deleteDaemonData = () => (dispatch, getState) => {
  ...
  dispatch({ type: DELETE_FNOD_ATTEMPT });
  wallet.deleteDaemonData(appData, isTestNet(getState()))
    .then(() => {
      dispatch({ type: DELETE_FNOD_SUCCESS });
      dispatch(shutdownApp());
    })
    .catch((err) => dispatch({ err, type: DELETE_FNOD_FAILED }));
};
...
export const getFnowalletLogs = () => {
  wallet.getFnowalletLogs()
    .then(logs => {
      return(logs);
    }).catch(
      err=>{
...
export const getFoneroLogs = () => {
  wallet.getFoneroLogs()
    .then(logs => {
      return(logs);
    }).catch(
      err=>{
