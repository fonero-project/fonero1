...
import * as da from "../middleware/fnodataapi";
import { EXTERNALREQUEST_FNODATA, EXTERNALREQUEST_POLITEIA } from "main_dev/externalRequests";
...
export const getStartupWalletInfo = () => (dispatch) => {
  ...
  const fnodataEnabled = config.get("allowed_external_requests").indexOf(EXTERNALREQUEST_FNODATA) > -1;
...
export const getTransactions = () => async (dispatch, getState) => {
  ...
  // first, request unmined transactions. They always come first in fonero.
...
export const getTreasuryBalance = () => (dispatch, getState) => {
  ...
  const dURL = sel.fnodataURL(getState());
  da.getTreasuryInfo(dURL, treasuryAddress)
    .then(treasuryInfo => {
      // Manually convert FNO to atom amounts to avoid floating point multiplication errors (eg. 589926.57667882*1e8 => 58992657667881.99)
      const treasuryBalance = parseInt(treasuryInfo["data"]["fno_unspent"].toString().replace(".",""));
