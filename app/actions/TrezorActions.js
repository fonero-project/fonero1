...
import { model1_fonero_homescreen } from "helpers/trezor";
...
// Right now (2018-07-06) fnowallet only supports a single account on watch only
...
// checkTrezorIsFnowallet verifies whether the wallet currently running on
// fnowallet (presumably a watch only wallet created from a trezor provided
...
// trezor wallet and then validating this address agains fnowallet, ensuring
...
const checkTrezorIsFnowallet = (session) => async (dispatch, getState) => {
  ...
  if (!addrValidResp.getIsMine()) throw "Trezor and fnowallet not running from the same extended public key";
...
export const signTransactionAttemptTrezor = (rawUnsigTx, constructTxResponse) => async (dispatch, getState) => {
  ...
  try {
    ...
    const signedRaw = await deviceRun(dispatch, getState, device, async session => {
      await dispatch(checkTrezorIsFnowallet(session));
...
// walletTxToBtcjsTx converts a tx decoded by the fonero wallet (ie,
...
export const walletTxToBtcjsTx = (tx, changeIndex, inputTxs) => async (dispatch, getState) => {
  ...
  for (const inp of tx.getInputsList()) {
    ...
    inputs.push({
      ...
      address_n: addressPath(addrIndex, addrBranch, WALLET_ACCOUNT,
        chainParams.HDCoinType),
      fonero_tree: inp.getTree()

      // FIXME: this needs to be supported on trezor.js.
      // foneroTree: inp.getTree(),
      // foneroScriptVersion: 0,
    });
  }
  ...
  for (const outp of tx.getOutputsList()) {
    ...
    outputs.push({
      ...
      address: addr,
      address_n: address_n,
      fonero_script_version: outp.getVersion(),
    });
  }
...
// walletTxToRefTx converts a tx decoded by the fonero wallet into a trezor
...
export const changeToFoneroHomeScreen = () => async (dispatch, getState) => {
  const device = selectors.trezorDevice(getState());
  if (!device) {
    dispatch({ error: "Device not connected", type:  TRZ_TOGGLEPASSPHRASEPROTECTION_FAILED });
    return;
  ...
  try {
    await deviceRun(dispatch, getState, device, async session => {
      await session.changeHomescreen(model1_fonero_homescreen);
    });
