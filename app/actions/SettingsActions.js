...
import { EXTERNALREQUEST_FNODATA } from "main_dev/externalRequests";
...
export const saveSettings = (settings) => (dispatch, getState) => {
  ...
  const oldFnodataEnabled = oldAllowedExternalRequests.indexOf(EXTERNALREQUEST_FNODATA) > -1;
  const newFnodataEnabled = settings.allowedExternalRequests.indexOf(EXTERNALREQUEST_FNODATA) > -1;
  if (newFnodataEnabled === true && oldFnodataEnabled === false) {
    dispatch(getTreasuryBalance());
  }
  if (newFnodataEnabled === false && oldFnodataEnabled === true) {
