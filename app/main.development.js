...
import { createLogger, lastLogLine, GetFnodLogs, GetFnowalletLogs } from "./main_dev/logging";
...
import { cleanShutdown, GetFnodPID, GetFnowPID } from "./main_dev/launch";
...
logger.log("info", "Versions: Fonero: %s, Electron: %s, Chrome: %s",
...
ipcMain.on("clean-shutdown", async function(event){
  const stopped = await cleanShutdown(mainWindow, app, GetFnodPID(), GetFnowPID());
...
ipcMain.on("get-fnod-logs", (event) => {
  event.returnValue = GetFnodLogs();
});

ipcMain.on("get-fnowallet-logs", (event) => {
  event.returnValue = GetFnowalletLogs();
});

ipcMain.on("get-fonero-logs", (event) => {
...
ipcMain.on("get-last-log-line-fnod", event => {
  event.returnValue = lastLogLine(GetFnodLogs());
});

ipcMain.on("get-last-log-line-fnowallet", event => {
  event.returnValue = lastLogLine(GetFnowalletLogs());
});
...
app.on("ready", async () => {
  ...
  // OS locale and set that as fonero's locale.
  ...
  windowOpts.title = "Fonero - " + app.getVersion();
...
app.on("before-quit", (event) => {
  logger.log("info","Caught before-quit. Set foneroition as was closed");
  event.preventDefault();
  cleanShutdown(mainWindow, app, GetFnodPID(), GetFnowPID());
