...
import { EXTERNALREQUEST_STAKEPOOL_LISTING, EXTERNALREQUEST_POLITEIA, EXTERNALREQUEST_FNODATA } from "main_dev/externalRequests";
import { POLITEIA_URL_TESTNET, POLITEIA_URL_MAINNET } from "./middleware/politeiaapi";
import { FNODATA_URL_TESTNET, FNODATA_URL_MAINNET } from "./middleware/fnodataapi";
...
export const currencies = () => [ { name: "FNO" }, { name: "atoms" } ];
...
export const unitDivisor = compose(disp => disp === "FNO" ? 100000000 : 1, currencyDisplay);
...
export const txURLBuilder= createSelector(
  [ network ],
  (network) =>
    (txHash) => `https://${network !== "testnet" ? "explorer" : network}.fnodata.org/${network == "testnet" ? "explorer/" : ""}tx/${txHash}`
);

export const blockURLBuilder= createSelector(
  [ network ],
  (network) =>
    (txHash) => `https://${network !== "testnet" ? "explorer" : network}.fnodata.org/${network == "testnet" ? "explorer/" : ""}block/${txHash}`
);
...
export const fnodataURL = createSelector(
  [ isTestNet ],
  (isTestNet) => isTestNet ? FNODATA_URL_TESTNET : FNODATA_URL_MAINNET
);
...
export const fnodataEnabled = compose(
  l => l.indexOf(EXTERNALREQUEST_FNODATA) > -1,
  allowedExternalRequests
);
