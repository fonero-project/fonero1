...
let fnodLogs = Buffer.from("");
let fnowalletLogs = Buffer.from("");
...
// format compatible to fnod/fnowallet logs. This function is meant to be
...
// fonero app data dir and sends to the console when debug == true.
  ...
  logger = new (winston.Logger)({
    transports: [
      new (winston.transports.File)({
        json: false,
        filename: path.join(appDataDirectory(), "fonero.log"),
...
export const AddToFnodLog = (destIO, data, debug) => {
  fnodLogs = AddToLog(destIO, fnodLogs, data, debug);
};

export const AddToFnowalletLog = (destIO, data, debug) => {
  fnowalletLogs = AddToLog(destIO, fnowalletLogs, data, debug);
};

export const GetFnodLogs = () => fnodLogs;

export const GetFnowalletLogs = () => fnowalletLogs;
...
export function ClearFnowalletLogs() {
  fnowalletLogs = Buffer.from("");
}
