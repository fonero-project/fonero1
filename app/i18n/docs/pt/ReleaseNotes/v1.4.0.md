Essa versão da fonero foi majoritariamente dedicada a corrigir alguns
...
suporte inclui a possibilidade de criar transações na fonero e então
...
da fonero e nessa versão adicionamos uma prova de conceito para temas
...
- Adicionada primeira versão da integração com dispositivos Trezor. Isso permite
  que os usuários utilizem a fonero como uma carteira apenas de visualização
...
- Fonero foi modificada para utilizar a versão 2 do serviço de compra
  automática de tickets da carteira (ao invés do antigo v1). Dado que após a
  [DCP-0001](https://github.com/fonero-project/dcps/blob/master/dcp-0001/dcp-0001.mediawiki)
...
- Stream de sincronização do inicializador. Agora os dois modos de sincronia
  ...
  Isso permite que a fnowallet seja responsável por realizar todos os passos de
...
- Todos os usuários irão ver uma nova página ao inicializar a carteira pela
  ...
  explica o funcionamento do [modo SPV](https://docs.fonero.org/wallets/spv).
...
- Quando um pânico é detectado pela fonero no fnod ou fnowallet, usuários
...
- A taxa de transação mínima padrão foi atualizada no fnod a algumas versões e
  agora a rede já tem uma quantidade suficiente de nós atualizados para permitir
  que as carteiras utilizem a taxa mínima de 0.0001 FNO/kB por padrão. Qualquer
  transação ou ticket criada no fonero ou fnowallet deve usar essa taxa
  mínima agora.
...
- Logs da fonero foram adicionadas na aba Logs, na página de ajuda.

- Com a adição do comando `getblockchaininfo` no fnod, agora podemos determinar
  o progresso da sincronização do fnod. Isso remove a necessidade de consultar
  fontes externas para determinar a situação da sincronização.

- Transações agora são decodificadas dentro da fonero sem precisar serem
  enviadas de volta para a fnowallet. Isso deve ajudar com o desempenho ao
