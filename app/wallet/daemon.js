...
export const checkFoneroVersion = log(() => Promise
  .resolve(ipcRenderer.sendSync("check-version"))
, "Check Fonero release version");
...
export const startWallet = log((walletPath, testnet) => new Promise((resolve, reject) => {
  ...
  // resolveCheck must be done both on the fnowallet-port event and on the
  ...
  ipcRenderer.once("fnowallet-port", (e, p) => { port = p; resolveCheck(); });
...
export const getFnodLogs = () => Promise
  .resolve(ipcRenderer.sendSync("get-fnod-logs"))
  .then(logs => {
    if (logs) return logs;
    throw "Error getting fnod logs";
  });

export const getFnowalletLogs = () => Promise
  .resolve(ipcRenderer.sendSync("get-fnowallet-logs"))
  .then(logs => {
    if (logs) return logs;
    throw "Error getting fnowallet logs";
  });

export const getFoneroLogs = () => Promise
  .resolve(ipcRenderer.sendSync("get-fonero-logs"))
  .then(logs => {
    if (logs) return logs;
    throw "Error getting fonero logs";
  });
...
export const getFnodLastLogLine = () => Promise
  .resolve(ipcRenderer.sendSync("get-last-log-line-fnod"));

export const getFnowalletLastLogLine = () => Promise
  .resolve(ipcRenderer.sendSync("get-last-log-line-fnowallet"));
