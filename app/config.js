...
import { appDataDirectory, getGlobalCfgPath, fnodCfg, getWalletPath, fnowalletCfg, getFnodRpcCert } from "./main_dev/paths";
...
export function initWalletCfg(testnet, walletPath) {
  ...
  if (!config.has("currency_display")) {
    config.set("currency_display","FNO");
  }
...
export function getWalletCert(certPath) {
  var cert;
  certPath = getFnodRpcCert(certPath);
  try {
...
export function readFnodConfig(configPath, testnet) {
  try {
    let readCfg;
    if (fs.existsSync(fnodCfg(configPath))) {
      readCfg = ini.parse(Buffer.from(fs.readFileSync(fnodCfg(configPath))).toString());
    } else if (fs.existsSync(fnodCfg(appDataDirectory()))) {
      readCfg = ini.parse(Buffer.from(fs.readFileSync(fnodCfg(appDataDirectory()))).toString());
    } else {
      return;
    }
    let newCfg = {};
...
export function getFnodCert(fnodCertPath) {
  if(fnodCertPath)
    if(fs.existsSync(fnodCertPath))
      return fs.readFileSync(fnodCertPath);

  var certPath = getFnodRpcCert();
...
export function createTempFnodConf() {
  if (!fs.existsSync(fnodCfg(appDataDirectory()))) {
    ...
    fs.writeFileSync(fnodCfg(appDataDirectory()), ini.stringify(fnodConf));
  }
...
export function newWalletConfigCreation(testnet, walletPath) {
  ...
  fs.writeFileSync(fnowalletCfg(getWalletPath(testnet, walletPath)), ini.stringify(fnowConf));
}
